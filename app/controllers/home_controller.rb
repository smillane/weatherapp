class HomeController < ApplicationController
  ...
  def find_aqi_zip(zip)
    request_aqi_api("https://www.airnowapi.org/aq/observation/zipCode/current/?format=application/json&zipCode=#{(zip)}&distance=25&API_KEY=#{(aqi_api_key)}")
  end

  def find_weather_zip(zip)
    request_weather_api("https://api.openweathermap.org/data/2.5/forecast?zip=#{(zip)},us&appid=#{(weather_api_key)}")
  end
